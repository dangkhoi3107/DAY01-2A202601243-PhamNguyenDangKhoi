...
def compare_models(prompt: str) -> dict:
    ...
    estimated_output_tokens = len(gpt4o_response.split()) / 0.75  # 1 token ~= 0.75 w

    gpt4o_cost_estimate = (
        estimated_output_tokens
        / 1000
        * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]  # USD/ 1000 token
    )
...
def run_assistant(
    ...
    while True:
        ...
        user_tokens = count_tokens(user_msg, OPENAI_MODEL)
        assistant_tokens = count_tokens(reply, OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    def shorten(text: str, max_length: int = 40) -> str:
        text = str(text)
